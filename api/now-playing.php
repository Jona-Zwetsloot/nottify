...
if (!empty($result['recenttracks']) && !empty($result['recenttracks']['track']) && !empty($result['recenttracks']['track'][0])) {
    ...
    echo '<div class="music-track"><div class="progress" data-step="' . ($isPlaying ? (1000 / $track['duration'] * 100) : 0) . '" data-width="' . ($isPlaying ? $progressWidth : 0) . '"></div><img src="api/image-proxy?url=' . filter_var(rawurlencode($track['image'][2]['#text']), FILTER_SANITIZE_SPECIAL_CHARS) . '"><div><h3 title="' . filter_var($track['name'], FILTER_SANITIZE_SPECIAL_CHARS) . '"><a href="' . filter_var($track['url'], FILTER_SANITIZE_SPECIAL_CHARS) . '">' . filter_var($track['name'], FILTER_SANITIZE_SPECIAL_CHARS) . '</a></h3><p>' . $artists . '</p></div><p>' . ($isPlaying ? 'now' : (isset($track['date'], $track['date']['uts']) ? getDateString($track['date']['uts']) : '')) . '</p></div>';
}
