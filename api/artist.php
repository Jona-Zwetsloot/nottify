...
if (file_exists($artistFile)) {
    ...
    if (!json_validate($content)) {
        exit(json_encode(['error' => str_replace('<file>', preg_replace($config['folder_regex'], '', $artistName) . '.json', text('invalid_json'))]));
    }
...
if (!array_key_exists('prevent_api_abuse', $config) || $config['prevent_api_abuse']) {
    ...
    if (!$artistInLibrary) {
        exit(json_encode(['error' => text('request_blocked')]));
    }
}
...
$musicbrainzEnabled = isset($config['artist_info_providers']) && in_array('musicbrainz', $config['artist_info_providers']);
$geniusEnabled = isset($genius, $genius['enabled'], $genius['apikey']) && $genius['enabled'] && isset($config['artist_info_providers']) && in_array('genius', $config['artist_info_providers']);
$spotifyEnabled = isset($spotify, $spotify['enabled'], $spotify['client_id'], $spotify['client_secret'], $spotify['redirect_uri']) && $spotify['enabled'] && !empty($_SESSION['spotify_token']) && isset($config['artist_info_providers']) && in_array('spotify', $config['artist_info_providers']);

// Add Genius data
if (!in_array('genius', $artist['sources']) && $geniusEnabled) {
        ...
        if (isset($geniusId)) {
                ...
                foreach ($search['response']['hits'] as $hit) {
                    ...
                    if ($isFromArtist) {
                        array_push($artist['popular'], [
                            'image' => 'api/image-proxy?url=' . rawurlencode($track['song_art_image_thumbnail_url']),
            ...
            if (isset($artistInfo['response'], $artistInfo['response']['artist'])) {
                ...
                if (isset($artistInfo['response']['artist']['description'], $artistInfo['response']['artist']['description']['dom'])) {
                    ...
                    if ($artist['description'] == '<div id="description"><p>?</p></div>') {
    ...
    writeFile($artistFile, json_encode($artist));
    $artist['continue'] = $spotifyEnabled;
    exit(json_encode($artist));
}

// Add Musicbrainz data
if (!in_array('musicbrainz', $artist['sources']) && $musicbrainzEnabled) {
    ...
    if (isset($search['artists'], $search['artists'][0], $search['artists'][0]['id'])) {
        ...
        if (isset($artistInfo['begin-area'], $artistInfo['begin-area']['name'])) {
            $artist['facts']['started_in'] = $artistInfo['begin-area']['name'];
        }
        if (isset($artistInfo['end-area'], $artistInfo['end-area']['name'])) {
            $artist['facts']['ended_in'] = $artistInfo['end-area']['name'];
        }
        ...
        if (!$geniusEnabled && !$spotifyEnabled && isset($artistInfo['releases']) && empty($tracks)) {
    ...
    writeFile($artistFile, json_encode($artist));
    $artist['continue'] = $geniusEnabled || $spotifyEnabled;
    exit(json_encode($artist));
}

// Add Spotify data
if (!in_array('spotify', $artist['sources']) && $spotifyEnabled) {
